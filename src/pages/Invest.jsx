...
function Invest() {
  ...
  const handleInvestClick = (pkg) => {
    ...
    setSelectedModalTermMonths(6); // Reset về mặc định khi mở modal
  ...
  const handleConfirmInvest = async () => {
    ...
    if (balanceToUse < selectedPackage.price) {
      toast.error(`Số dư ${selectedCurrency} không đủ để đầu tư gói này`);
      return;
    }
  ...
  return (
    <div className="space-y-4">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-6">
          ...
          {packagesLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {packages.map((pkg) => {
                ...
                return (
                  <div key={pkg.id} className={`bg-slate-700/50 rounded-lg border ${colors.border} p-6 glow-border glow-border-hover transition-all hover:shadow-lg`}>
                    ...
                    <div className="mb-4">
                      <p className="text-2xl font-bold text-emerald-300 mb-1">{formatCurrency(pkg.price, 'USDT')}</p>
                      <p className="text-xs text-emerald-300/60">Minimum investment</p>
                    </div>

                    {pkg.description && (
                      <p className="text-sm text-emerald-300/80 mb-4">{pkg.description}</p>
                    )}

                    {!hasEnoughBalance && (
                      <p className="text-xs text-red-400 mb-4">⚠️ Insufficient balance</p>
                    )}
                    ...
                    <button
                      ...
                      className={`w-full py-2.5 px-4 rounded-lg font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                        hasEnoughBalance
                          ...
                          : 'bg-slate-600/50 text-emerald-300/50 cursor-not-allowed'
                      }`}
                    >
        ...
        {/* Investment History */}
        {/* Giữ nguyên phần lịch sử như cũ - không thay đổi */}
        <div>
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-emerald-500/20 rounded-full p-2">
              <svg className="w-6 h-6 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <h2 className="text-lg font-semibold text-emerald-400 uppercase">Investment History</h2>
          </div>
          ...
          <div className="bg-slate-700/50 rounded-lg border border-emerald-500/50 p-4 glow-border glow-border-hover">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {historyLoading && (
                    <tr><td colSpan={6} className="py-4 px-4 text-center text-xs text-emerald-300/80">Loading history...</td></tr>
                  )}
                  {!historyLoading && historyError && (
                    <tr><td colSpan={6} className="py-4 px-4 text-center text-xs text-red-300">{historyError}</td></tr>
                  )}
                  {!historyLoading && !historyError && history.length === 0 && (
                    <tr><td colSpan={6} className="py-4 px-4 text-center text-xs text-emerald-300/80">No investment history available.</td></tr>
                  )}
                  {!historyLoading && !historyError && history.map((item) => (
                    <tr key={item.id} className="border-b border-emerald-500/10 hover:bg-emerald-500/5 transition-colors">
                      <td className="py-3 px-4 text-xs text-emerald-300/80">
                        {(() => {
                          const timestamp = Number(item.startDate || item.startDate);
                          if (isNaN(timestamp) || timestamp <= 0) return <span className="text-red-400">Invalid Date</span>;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-700 dark:bg-gray-800 rounded-lg border border-emerald-500/50 p-6 w-full max-w-md glow-border">
            ...
            <div className="space-y-4 mb-6">
              ...
              {/* Chọn Term ngay trong Modal */}
              <div>
                <label className="block text-sm text-emerald-300/80 mb-2">
                  Investment Term
                </label>
                <select
              ...
              <div>
                <p className="text-sm text-emerald-300/80 mb-1">Amount</p>
                <p className="font-semibold text-emerald-400">{formatCurrency(selectedPackage.price, selectedCurrency)}</p>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {['USDT'].map((cur) => {  // Nếu sau này có NOVA thì thêm vào đây
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-sm mt-1">
                        ...
                        {!hasBalance && <span className="text-red-400 ml-2">Insufficient</span>}
                      </div>
                    </button>
                  );
                })}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={investing || (selectedCurrency === 'USDT' && usdtBalance < selectedPackage.price)}
                className={`flex-1 py-3 px-4 rounded-lg font-semibold transition-all ${
                  !investing && (selectedCurrency === 'USDT' && usdtBalance >= selectedPackage.price)
